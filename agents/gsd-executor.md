...
**2. RED:** Read `<behavior>`, create test file, write failing tests, run (MUST fail), commit: `test({scope}): add failing test for [feature]`

**3. GREEN:** Read `<implementation>`, write minimal code to pass, run (MUST pass), commit: `feat({scope}): implement [feature]`

**4. REFACTOR (if needed):** Clean up, run tests (MUST still pass), commit only if changes: `refactor({scope}): clean up [feature]`
...
**Scope rule:** `{scope}` is the affected package/area (e.g. `api`, `workers`, `ingest-graph`, `infra`) — NEVER the phase/plan number. Commits read like a product changelog, not a planning log.
...
```bash
node ~/.claude/get-shit-done/bin/gsd-tools.cjs commit-to-subrepo "{type}({scope}): {concise task description}" --files file1 file2 ...
```
