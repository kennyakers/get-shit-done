...
```bash
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  # Compute diff base from the phase branch range — fail closed if none found.
  # GSD commits use semantic area scopes, not a plan-number tag to grep; the
  # fork point of the phase branch is the reliable base for the phase's diff.
  DIFF_BASE=$(git merge-base HEAD "@{upstream}" 2>/dev/null || git merge-base HEAD main 2>/dev/null || git merge-base HEAD master 2>/dev/null)

  if [ -n "$DIFF_BASE" ]; then
    # Run git diff with specific exclusions
    ...
    echo "Warning: Could not determine a diff base (no upstream/main/master). Cannot determine reliable diff scope."
...
```bash
# Phase work lives on the phase branch — base is its fork point (GSD commits use semantic scopes, not a phase-number tag)
DIFF_BASE=$(git merge-base HEAD "@{upstream}" 2>/dev/null || git merge-base HEAD main 2>/dev/null || git merge-base HEAD master 2>/dev/null)
```
