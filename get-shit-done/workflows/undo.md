...
Filter for conventional commits matching `type(scope): message` (e.g., `feat(api):`, `docs(workers):`, `fix(infra):`). GSD commits use semantic area scopes — identify phase work by the current phase branch, not a plan-number tag.

Display a numbered list of matching commits:

```
Recent GSD commits:
  1. abc1234 feat(api): implement auth endpoint
  2. def5678 docs(products): complete plan summary
  3. ghi9012 fix(auth): correct validation logic
```
...
- Display: "Manifest has no entry for phase ${TARGET_PHASE} (or file missing), falling back to the phase branch range"
- Fallback: GSD commits use semantic area scopes (not a phase-number tag), so isolate the phase's work by the phase branch range — the commits on the current branch since it forked:
  ```bash
  BASE=$(git merge-base HEAD "@{upstream}" 2>/dev/null || git merge-base HEAD main 2>/dev/null || git merge-base HEAD master 2>/dev/null)
  [ -n "$BASE" ] && git log --oneline --no-merges "${BASE}..HEAD" | head -50
  ```
- Use matching commits as COMMITS. (Note: this selects all commits on the phase branch; if multiple phases share one branch, prefer the manifest for exact per-phase isolation.)

---

**MODE=plan:**

Prefer the manifest: if `.planning/.phase-manifest.json` records commit hashes for the plan's phase, use them. Otherwise — since GSD commits no longer carry a per-plan tag to grep — fall back to the phase branch range and let the user select which commits belong to the plan:
...
Present the list for interactive selection (plan-level isolation without the manifest is best-effort). Use the selected commits as COMMITS.
